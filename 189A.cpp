...
int f(int n, int a, int b, int c, UMAPII &hash) {
    if(n==0)
        return hash[0] = 1;
    ...
    if(n<0)
        return INT_MIN;
    int x,y,z;
    x = n>=a ? 1+f(n-a,a,b,c,hash) : INT_MIN;
    y = n>=b ? 1+f(n-b,a,b,c,hash) : INT_MIN;
    z = n>=c ? 1+f(n-c,a,b,c,hash) : INT_MIN;
    return hash[n] = max_3(x,y,z);
}
