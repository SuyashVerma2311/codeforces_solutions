...
void solve(int t) {
	...
	while(t--) {
		...
		int as=0, bs=0, cs=0;
		VI a_, b_, c_, a_cnt(n,0), b_cnt(n,0), c_cnt(n,0);
		...
		if(a_.size() >= 2) {
			loop(i,a_.size()-1) {
				as = a_cnt[a_[i+1]]-a_cnt[a_[i]]+1;
				bs = b_cnt[a_[i+1]]-b_cnt[a_[i]];
				cs = c_cnt[a_[i+1]]-c_cnt[a_[i]];

				if(as > bs && as > cs) {
					did = true;
					output = min(output, (a_[i+1] - a_[i] + 1));
				}
			}
		}
		if(a_.size() >= 3) {
			loop(i,a_.size()-2) {
				if(a_[i+2]==a_[i+1]+3 && a_[i+1]==a_[i]+3 && b_cnt[a_[i+2]]-b_cnt[a_[i]]==2) {
					did = true;
					output = min(output, 7);
				}				
			}
